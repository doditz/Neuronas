...
def make_replit_blueprint():
    try:
        ...
        app.logger.error("Le REPL_ID n'est pas défini, l'authentification Replit ne sera pas disponible")
        return None
    ...
    def error():
        flash("Une erreur d'authentification s'est produite", "danger")
...
def save_user(user_claims):
    ...
    if user is None:
        ...
        user.username = user_claims.get('email', '').split('@')[0] if user_claims.get('email') else f"user_{user_claims['sub']}"
...
def handle_error(blueprint, error, error_description=None, error_uri=None):
    ...
    app.logger.error(f"Erreur OAuth avec Replit: {error}, {error_description}")
    flash(f"Erreur d'authentification: {error_description or error}", "danger")
    return redirect(url_for('index'))
